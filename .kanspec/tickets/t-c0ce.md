---
id: t-c0ce
title: 'Python src/ layout: imports from tests/ never resolve into src/<pkg>/ (roots are only the importer''s ancestors), so on Sentry 0 of the top 15 files show a test reference and every score gets the no-tests boost; read package_dir/pyproject or try a src/ root'
state: doing
spec: null
proposal: null
item: null
deps: []
followup_of: null
discovered_in: t-02b6
branch: ks/t-c0ce-python-src-layout-imports-from-tests-nev
worktree: null
claimed_by: claude/7b7df9fd-509a
pr: null
head: null
spec_unchanged: null
created: 2026-09-14T01:10:37Z
---
Python src/ layout: imports from tests/ never resolve into src/<pkg>/ (roots are only the importer's ancestors), so on Sentry 0 of the top 15 files show a test reference and every score gets the no-tests boost; read package_dir/pyproject or try a src/ root

## Steps

## Log
- 2026-09-14T01:10Z  todo     claude/7b7df9fd-509a  new (discovered in t-02b6)
- 2026-09-14T01:14Z  doing    claude/7b7df9fd-509a  start (branch created)
