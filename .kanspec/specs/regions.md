...
- Rust only. TS, TSX, JS and Python files never get regions. `[tests].inline_modules = false` turns the effects off (no unit tags, no line subtraction, no clone stripping); metrics still list the regions so `has_tests` sees an inline `mod tests` either way.
