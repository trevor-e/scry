...
- `analyze_all_with` hands each file's parsed tree (or `None`) and its test regions to a caller closure once, in file order: the mentions pass takes its symbols and inline test units there, so a Source file is parsed once for both.
