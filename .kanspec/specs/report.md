...
- The missing-tests reason reads "no test file imports it": it is an import/stem signal, not coverage, and a parent's test may still exercise the file. `has_tests` is true when a test file imports the file, when a test file's normalised stem equals the file's stem *and* the test sits in the same directory tree (the test's path above its first test-directory component is an ancestor of the file), or when a Rust file carries an inline `#[cfg(test)]` module.
