...
| fallback | fallback sites per function (`unwrap_or*` / `.ok()?`, Python `d.get(k, lit)` / `x or lit`, TS `?? lit`) and the *parse defaults* among them: a literal default (`""`, `0`, `Default::default()`) applied to a receiver chain holding a fallible transform (`parse`, `split`, `next`, `get`…); a function with 2+ such sites gets a reason naming every line and what each value becomes; no weight | an LLM patches a failed parse with the cheapest default instead of an error, so scry's own git-log `parse` turns a malformed header line into hash "", author "", ts 0 and subject "" at 5 sites and kanspec's `discover` turns a short `rev-parse` reply into an empty `git_dir`; no function in ripgrep, fd, click or hono has 2 such sites (they have 0, 2, 2 and 3 single ones), and an `unwrap_or` on a struct field has no call in its chain and never counts |
