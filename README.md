...
| metrics | cognitive + cyclomatic complexity, nesting, length, params and `locals` (distinct `let` / assignment / `for` / `with` / declarator names; a pattern is one, closures fold in, nested functions do not) per function; a unit that is 100+ lines, cognitive 15+ and 15+ locals at once is labelled a *brain method* on the cognitive reason with its 3 longest-lived locals, no weight | nesting-aware complexity predicts "hard to change safely"; an LLM fixes a long function by adding one more `let` instead of extracting, so kanspec's `close` (258 lines, cognitive 42) binds 37 names and `attention` 23, against one such unit in all of ripgrep and one in fd |
...
| comments | top-level banner comments (`// ── The ladder ──`, rule / title / rule triples; doc comments, editor `#region` markers and a top-of-file license header never count) partition a file into labelled sections with the units each holds, annotated on a hotspot once the file is 400+ lines with 3+ sections and a 100+ line largest one; inside a function already over the cognitive threshold, the phase labels its author wrote (`// ── step 1: verify merged ──`, `// 2. …`) become a sub-line of the metrics reason with each phase's span, its estimated cognitive as a helper and the locals the phases share; weight 0 | an LLM narrates sub-files inside one file and pre-annotates the phases of a long function, so the split points are in the source: kanspec's `scan.rs` is cut into 'The seals' / 'The ladder's value types' / 'The ladder' (665 lines) / 'The recorded human override', `derive.rs` into 6 sections and `cmd/flow.rs` into 7 verb sections, and `attention` (cognitive 30) already labels YOU / AGENT / WATCHING; ripgrep, fd, click and hono have no banner and no phase at all, and a plain "extract from ladder" step gave the agent nothing to cut along |
| naming | one-letter bindings (`let p`, `for t`, `const h`; `i`/`j`/`k`/`n`/`x`/`y`/`z` allowed) and the widest gap between two consecutive uses of each, with every `\|t\|` closure, `Some(t)` arm, `if let` / `for` pattern and lambda / comprehension target left out of the search and a later `let t` ending it; a binding whose widest gap reaches 30 lines is an attribute on a unit the report already ranks, never a score input; the one-letter share of all bindings per file in JSON | an LLM names a 258-line function's proposal `p` the way a human names a 5-line loop's, so the reader meets `p` on line 884 and scrolls to 690 to learn what it is: one-letter bindings are 19.6% of kanspec's and 20% of scry's bindings against 1.9-5.7% in hono, click, fd and ripgrep, and kanspec's `close` binds `p` at 690 and next reads it 122 lines later; the raw span the prototype measured is inflated by rebinding (`t` in `attention` is three separate `for t` loops) and by accumulators (`h` in `render_page_html` is pushed every few lines), so the gap variant flags 9 bindings in kanspec, 2 in scry and 1 in ripgrep |
| fallback | fallback sites per function (`unwrap_or*` / `.ok()?`, Python `d.get(k, lit)` / `x or lit`, TS `?? lit`) and the *parse defaults* among them: a literal default (`""`, `0`, `Default::default()`) applied to a receiver chain holding a fallible transform (`parse`, `split`, `next`, `strip_prefix`…; a map `get` with a default is the benign case and does not count); a function with 2+ such sites gets a reason naming every line and what each value becomes; no weight | an LLM patches a failed parse with the cheapest default instead of an error, so scry's own git-log `parse` turns a malformed header line into hash "", author "", ts 0 and subject "" at 5 sites and kanspec's `discover` turns a short `rev-parse` reply into an empty `git_dir`; no function in ripgrep, fd, click or hono has 2 such sites (they have 0, 2, 2 and 3 single ones), and an `unwrap_or` on a struct field has no call in its chain and never counts |
