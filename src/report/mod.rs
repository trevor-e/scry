...
use crate::clones::{CloneKind, CloneReport, ClonePair, CloneFamily, Loc, TableRef};
use crate::clumps::{self, Clump, ClumpsReport};
use crate::config::{Fallback as FallbackCfg, History as HistoryCfg, Naming as NamingCfg, Plan as PlanCfg, Report as Cfg, Tests as TestsCfg, Weights};
...
pub struct Signals {
    ...
    /// Union of lines in all four clone categories.
...
pub struct ScoreContribution {
    ...
    pub weight: f64,
    /// Score points before the test-proximity multiplier.
    pub points: f64,
}
...
pub struct Hotspot {
    ...
    /// The three highest-cognitive source units, each with its labelled `phases` (see
    /// `comments`) when it has enough of them.
    pub worst_functions: Vec<FunctionMetrics>,
    /// The file's labelled banner sections (see `comments`), filled when the banner
    /// annotation is printed for it; empty otherwise.
...
pub struct Report {
    ...
    pub structural_clones: Vec<ClonePair>,
...
pub struct Inputs<'a> {
    ...
    /// `[fallback]`: the parse-default floor, the exempt names and the per-file cap.
    pub fallback: &'a FallbackCfg,
    pub cognitive_hard: u32,
...
fn clone_reason(s: &Signals, tables: &[TableRef], path: &str, largest: Option<&ClonePair>) -> String {
    ...
    if s.signature_clone_lines + s.configuration_clone_lines > 0 {
        return format!("{:.0}% weighted clone ratio: {} logic, {} table, {} signature, {} configuration lines (signatures/configuration excluded from ranking){largest}",
            s.clone_ratio * 100.0, s.logic_clone_lines, s.table_clone_lines, s.signature_clone_lines, s.configuration_clone_lines);
    }
...
pub fn build(inp: Inputs, top: usize, cfg: &Cfg, test_dirs: &[String]) -> Report {
    ...
    let signals: Vec<Signals> = source
        ...
        .map(|f| {
            ...
            Signals {
                ...
                in_runtime_cycle: d.is_some_and(|d| d.in_runtime_cycle),
                ...
                signature_clone_lines: c.map_or(0, |c| c.signature_clone_lines),
                configuration_clone_lines: c.map_or(0, |c| c.configuration_clone_lines),
                weighted_clone_lines: c.map_or(0.0, |c| c.weighted_clone_lines),
    ...
    let p_clone = percentiles(&signals.iter().map(|s| s.weighted_clone_lines).collect::<Vec<_>>());
    ...
    let exempt = fallback::Exempt::new(inp.fallback);
    ...
    let mut hotspots: Vec<Hotspot> = source
        ...
        .map(|(i, f)| {
            ...
            let coupling = if s.fan_in == 0 { 0.0 } else { p_fanin[i] }.max(if s.in_runtime_cycle { cfg.cycle_coupling } else { 0.0 });
            let clone = if s.weighted_clone_lines == 0.0 { 0.0 } else { p_clone[i] };
            ...
            let contributions: Vec<_> = [
                ("hotspot", hotspot, w.hotspot), ("fixes", fix, w.fixes), ("complexity", cx, w.complexity),
                ("coupling", coupling, w.coupling), ("clones", clone, w.clones), ("size", p_lines[i], w.size),
                ("dead", dead_p, w.dead), ("helpers", helpers_p, inp.helpers_weight), ("strings", strings_p, w.strings),
                ("clumps", clumps_p, inp.clumps_weight), ("declared", declared_p, inp.declared_weight),
                ("naming", naming_p, inp.naming.weight_into_complexity),
            ].into_iter().map(|(name, value, weight)| ScoreContribution {
                signal: name.into(), normalized_value: value, weight, points: 100.0 * value * weight,
            }).collect();
            let subtotal = contributions.iter().map(|c| c.points).sum::<f64>();
            let test_multiplier = if s.test_units > 0 { 1.0 } else { cfg.no_tests_multiplier };
            let score = subtotal * test_multiplier;
            let score_breakdown = ScoreBreakdown { contributions, subtotal, test_multiplier, total: score };
            ...
            // Parse defaults (see `fallback`): one reason per production unit swallowing
            // `[fallback].min_sites`+ parse failures with literal defaults; never a score input.
            reasons.extend(fallback::file_reasons(units.get(f.path.as_str()).map_or(&[], Vec::as_slice), inp.fallback, &exempt));
            ...
            // Comments: the banner partition, when the file is big enough by percentile (the
            // printed hotspots below the percentile get theirs after the ranking).
            let mut sections = Vec::new();
            if let Some(c) = comments_file.filter(|c| c.qualifies && c.above_size_percentile) {
                reasons.extend(c.banner_reason.iter().cloned());
            ...
            Hotspot {
                ...
                signals: s.clone(),
                reasons,
                worst_functions: worst.get(f.path.as_str()).map(|v| v.iter().map(|w| {
                    let mut w = (*w).clone();
                    w.phases = comments_file.and_then(|c| c.units.iter().find(|u| u.unit == w.name && u.start_line == w.start_line)).map(|u| u.phases.clone()).unwrap_or_default();
                    w
    ...
    // A printed hotspot cut into labelled sections gets the annotation whatever its size percentile.
    for h in &mut hotspots {
        if let Some(c) = inp.comments.files.get(&h.path).filter(|c| c.qualifies && !c.above_size_percentile) {
            h.reasons.extend(c.banner_reason.iter().cloned());
            h.sections = c.sections.clone();
        }
    }
    ...
    Report {
        ...
        runtime_file_cycles: inp.deps.runtime_file_cycles.clone(),
        file_cycles: inp.deps.file_cycles.clone(),
        clone_families: inp.clones.families.iter().take(top).cloned().collect(),
        structural_clones: inp.clones.pairs.iter().filter(|p| matches!(p.kind, CloneKind::Signature | CloneKind::Configuration)).take(top).cloned().collect(),
        ...
        clones: inp.clones.pairs.iter().filter(|p| p.kind == CloneKind::Logic || (!inp.list_tables_separately && p.kind == CloneKind::Table)).take(top).cloned().collect(),
...
mod tests {
    ...
    fn nocomments() -> &'static CommentsReport {
        static R: std::sync::OnceLock<CommentsReport> = std::sync::OnceLock::new();
    ...
    fn nofallback() -> &'static FallbackCfg {
        static R: std::sync::OnceLock<FallbackCfg> = std::sync::OnceLock::new();
        R.get_or_init(FallbackCfg::default)
    }
    ...
    fn locals_ride_the_cognitive_reason_and_a_brain_method_is_a_label_only() {
        ...
        let unit = |file: &str, name: &str, s: usize, lines: usize, cognitive: u32, locals: usize, brain: bool| FunctionMetrics {
            ...
            parse_defaults: 0, fallbacks: 0, fallback_sites: Vec::new(),
        };
        ...
        assert!(json.contains("\"locals\":9,\"parse_defaults\":0"), "no brain / longest_locals keys on a plain unit: {json}");
    }
    ...
    fn short_name_attribute_rides_the_functions_reason_and_weighs_nothing_by_default() {
        ...
        let unit = |file: &str, name: &str, s: usize, lines: usize, cognitive: u32, short: Vec<ShortBinding>| {
            ...
            FunctionMetrics { file: file.into(), name: name.into(), start_line: s, end_line: s + lines - 1, lines, params: 0, cyclomatic: 1, cognitive, max_nesting: 1, in_test: false, phases: Vec::new(), bindings: 6, short_bindings: short, long_short_bindings: long, locals: 0, brain: false, longest_locals: Vec::new(), parse_defaults: 0, fallbacks: 0, fallback_sites: Vec::new() }
        };
        ...
        let inputs = |naming: &'static NamingCfg| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &functions, deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming, fallback: nofallback() };
    ...
    #[test]
    fn banner_sections_and_phase_sub_lines_annotate_hotspots_without_scoring() {
        use crate::comments::{FileComments, Phase, UnitPhases};
        let sf = |p: &str, lines: usize| SourceFile {
            path: p.into(), lang: crate::lang::Language::Rust, kind: FileKind::Source, lines, bytes: 0, content: String::new(),
        };
        // a.rs: over the percentile, with a phased unit. b.rs: below it, so its sections print only
        ...
        let functions = vec![FunctionMetrics { file: "a.rs".into(), name: "ladder".into(), start_line: 405, end_line: 656, lines: 252, params: 1, cyclomatic: 9, cognitive: 32, max_nesting: 5, in_test: false, phases: Vec::new(), bindings: 0, short_bindings: Vec::new(), long_short_bindings: 0, locals: 0, brain: false, longest_locals: Vec::new(), parse_defaults: 0, fallbacks: 0, fallback_sites: Vec::new() }];
        ...
        let mut comments = CommentsReport::default();
        comments.files.insert("a.rs".into(), FileComments { banners: 3, sections: vec![section("one", 2, 300), section("two", 302, 900), section("three", 902, 1400)], qualifies: true, size_percentile: 100.0, above_size_percentile: true, banner_reason: Some("a.rs is cut into 3 labelled sections: 'one' 2-300, 'two' 302-900 (599 lines, 1 fn), 'three' 902-1400 — extract the largest as two.rs".into()), phase_comments: 2, units: vec![unit] });
        for (p, pct) in [("b.rs", 50.0), ("c.rs", 0.0)] {
            comments.files.insert(p.into(), FileComments { banners: 3, sections: vec![section("x", 2, 300), section("y", 302, 600), section("z", 602, 800)], qualifies: true, size_percentile: pct, above_size_percentile: false, banner_reason: Some(format!("{p} is cut into 3 labelled sections: 'x' 2-300, 'y' 302-600 (299 lines, 1 fn), 'z' 602-800 — extract the largest as y.rs")), phase_comments: 0, units: vec![] });
        }
        let (deps, clones, tests, hcfg, pcfg, mentions) = (DepGraph::default(), CloneReport::default(), TestsCfg::default(), hcfg(), pcfg(), MentionIndex::default());
        let size_only = Cfg {
            without_history: Weights { hotspot: 0.0, fixes: 0.0, complexity: 0.0, coupling: 0.0, clones: 0.0, size: 1.0, dead: 0.0, strings: 0.0 },
            ..Cfg::default()
        };
        let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &functions, deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: &comments, naming: nonaming(), fallback: nofallback() }, 2, &size_only, &td());
        let a = &r.hotspots[0];
        ...
        // b.rs is below the percentile but printed: the annotation lands after the ranking.
        let b = &r.hotspots[1];
        assert_eq!((b.path.as_str(), b.sections.len()), ("b.rs", 3));
        assert!(b.reasons.iter().any(|x| x.starts_with("b.rs is cut into")), "{:?}", b.reasons);
        assert_eq!(r.hotspots.len(), 2, "c.rs is not printed at --top 2");
    ...
    fn inline_test_regions_shrink_size_and_count_as_tests() {
        ...
        let functions = vec![
            FunctionMetrics { file: "a.rs".into(), name: "src".into(), start_line: 1, end_line: 2, lines: 2, params: 0, cyclomatic: 1, cognitive: 1, max_nesting: 0, in_test: false, phases: Vec::new(), bindings: 0, short_bindings: Vec::new(), long_short_bindings: 0, locals: 0, brain: false, longest_locals: Vec::new(), parse_defaults: 0, fallbacks: 0, fallback_sites: Vec::new() },
            FunctionMetrics { file: "a.rs".into(), name: "tst".into(), start_line: 1300, end_line: 1360, lines: 61, params: 0, cyclomatic: 1, cognitive: 40, max_nesting: 3, in_test: true, phases: Vec::new(), bindings: 0, short_bindings: Vec::new(), long_short_bindings: 0, locals: 0, brain: false, longest_locals: Vec::new(), parse_defaults: 0, fallbacks: 0, fallback_sites: Vec::new() },
        ];
        let deps = DepGraph::default();
        let clones = CloneReport::default();
        let tests = TestsCfg::default();
        ...
        let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &functions, deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &size_only, &td());
    ...
    fn dead_ratio_is_a_weighted_signal_and_dead_reasons_reach_the_hotspot() {
        ...
        let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: &dead, helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &dead_only, &td());
        let paths: Vec<&str> = r.hotspots.iter().map(|h| h.path.as_str()).collect();
    ...
    fn helper_signals_are_weighted_by_the_helpers_knob_and_the_section_prints() {
        ...
        let inputs = |w: f64| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: &h, helpers_weight: w, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() };
        ...
        // Nothing indexed: the section says none.
        let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &none, &td());
    ...
    fn clump_signals_are_weighted_by_the_clumps_knob_and_the_section_prints() {
        ...
        let inputs = |w: f64| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: &c, clumps_weight: w, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() };
        let r = build(inputs(1.0), 10, &none, &td());
        let paths: Vec<&str> = r.hotspots.iter().map(|h| h.path.as_str()).collect();
    ...
    fn unread_knob_reasons_reach_the_config_file_and_the_declared_section_prints() {
        ...
        let inputs = |w: f64| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: &d, declared_weight: w, comments: nocomments(), naming: nonaming(), fallback: nofallback() };
        let r = build(inputs(1.0), 10, &none, &td());
        ...
        let inputs2 = |w: f64| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: &d2, declared_weight: w, comments: nocomments(), naming: nonaming(), fallback: nofallback() };
    ...
    fn a_file_touched_only_by_bots_has_bus_factor_zero() {
        ...
        let (deps, clones, tests, hcfg, mentions, pcfg) = (DepGraph::default(), CloneReport::default(), TestsCfg::default(), hcfg(), MentionIndex::default(), pcfg());
        let r = build(Inputs { root: String::new(), files: &files, history: Some(&hist), file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &Cfg::default(), &td());
    ...
    fn table_pairs_list_under_tables_and_the_clone_reason_states_the_split() {
        ...
        let mut clones = CloneReport { pairs: vec![logic, table], files: HashMap::new(), ..Default::default() };
        clones.files.insert("a.rs".into(), FileClones {
            clone_lines: 115, logic_clone_lines: 31, table_clone_lines: 84, weighted_clone_lines: 115.0, signature_clone_lines: 0, configuration_clone_lines: 0, clone_ratio: 0.23, pairs: 3,
        ...
        let pcfg = pcfg();
        let mentions = MentionIndex::default();
        let inputs = |sep: bool| Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: sep, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() };
        ...
        // Tables stay visible in the report but do not propose merging public operations.
        let plan: Vec<&str> = a.plan.iter().map(|s| s.text.as_str()).collect();
        assert_eq!(plan, vec!["run (200-230) shares logic with b.rs go (10-40), 150 tokens; compare differences before extracting a helper"]);
        assert!(text.contains("        PLAN\n          1) run (200-230) shares logic with b.rs go (10-40), 150 tokens; compare differences before extracting a helper\n"), "{text}");
    ...
    fn test_clone_runs_fold_into_one_step_and_never_canonicalise() {
        ...
        assert_eq!(a.plan.iter().map(|s| s.text.as_str()).collect::<Vec<_>>(), vec![
            ...
            "alpha (10-20) shares logic with c.rs t_far (50-60), 90 tokens; compare differences before extracting a helper",
            "gamma (30-40) shares logic with b.rs delta (5-15), 80 tokens; compare differences before extracting a helper",
        ], "{:?}", a.plan);
        assert_eq!((a.plan[0].kind, a.plan_more), (StepKind::FoldTestClones, 0));
        let b = r.hotspots.iter().find(|h| h.path == "b.rs").unwrap();
        assert_eq!(b.plan.iter().map(|s| s.text.as_str()).collect::<Vec<_>>(), vec!["delta (5-15) shares logic with a.rs gamma (30-40), 80 tokens; compare differences before extracting a helper"]);
        ...
        let a = r.hotspots.iter().find(|h| h.path == "a.rs").unwrap();
        assert_eq!(a.plan.iter().map(|s| s.text.as_str()).collect::<Vec<_>>(), vec!["alpha (10-20) shares logic with c.rs t_far (50-60), 90 tokens; compare differences before extracting a helper", "gamma (30-40) shares logic with b.rs delta (5-15), 80 tokens; compare differences before extracting a helper"]);
    }
    ...
    fn cycle_reason_is_deduped_and_cycles_print_one_block_each() {
        ...
        deps.runtime_file_cycles = vec![deps.file_cycles[0].members.clone(), deps.file_cycles[1].members.clone(), members("src/jsx", 4), deps.file_cycles[4].members.clone()];
        let clones = CloneReport::default();
        let tests = TestsCfg::default();
        let hcfg = hcfg();
        let pcfg = pcfg();
    ...
    #[test]
    fn weighted_clone_lines_change_ranking_and_breakdown_reconstructs_score() {
        let sf = |path: &str, lang, content: String| SourceFile { path: path.into(), lang, kind: FileKind::Source, lines: content.lines().count(), bytes: content.len(), content };
        let registry = |name: &str| format!("static {name}: &[Entry] = &[\n{}];\n", (0..24).map(|i| format!("Entry {{ id: {i}, label: \"item\", action: run_{i}, enabled: true }},\n")).collect::<String>());
        let logic = |name: &str| format!("def {name}(items):\n    total = 0\n    for item in items:\n        if item['ready']:\n            total += item['weight'] * 3\n        else:\n            total -= item['penalty']\n    if total > 10:\n        emit(total, items[0], True)\n    while total < 0:\n        total += 2\n    return max(total, 0)\n");
        let files = [sf("table_a.rs", crate::lang::Language::Rust, registry("A")), sf("table_b.rs", crate::lang::Language::Rust, registry("B")),
            sf("logic_a.py", crate::lang::Language::Python, logic("alpha")), sf("logic_b.py", crate::lang::Language::Python, logic("beta"))];
        let fm: Vec<_> = files.iter().map(|f| fmetrics(&f.path, 1, 1, 0)).collect();
        let (deps, tests, hcfg, mentions, pcfg) = (DepGraph::default(), TestsCfg::default(), hcfg(), MentionIndex::default(), pcfg());
        let only_clones = Cfg { without_history: Weights { hotspot: 0.0, fixes: 0.0, complexity: 0.0, coupling: 0.0, clones: 1.0, size: 0.0, dead: 0.0, strings: 0.0 }, no_tests_multiplier: 1.2, ..Cfg::default() };
        for weight in [1.0, 0.01, 0.0] {
            let cfg = crate::config::Clones { table_weight: weight, min_tokens: 40, ..Default::default() };
            let clones = crate::clones::detect(&files, &cfg, &tests, pcfg.symbol_fallback);
            assert!(clones.files["table_a.rs"].table_clone_lines > 0);
            assert!(clones.files["logic_a.py"].logic_clone_lines > 0);
            let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &only_clones, &td());
            let score = |path| r.hotspots.iter().find(|h| h.path == path).unwrap().score;
            assert_eq!(score("table_a.rs") > score("logic_a.py"), weight == 1.0, "weight {weight}: {:?}", r.hotspots);
            if weight == 0.0 { assert_eq!(score("table_a.rs"), 0.0); }
            for h in &r.hotspots {
                let b = &h.score_breakdown;
                let sum = b.contributions.iter().map(|c| { assert_eq!(c.points, 100.0 * c.normalized_value * c.weight); c.points }).sum::<f64>();
                assert!((sum * b.test_multiplier - h.score).abs() < 1e-9);
                assert_eq!(b.total, h.score);
                assert_eq!(b.subtotal, sum);
            }
        }
    }

    #[test]
    fn structural_cycle_does_not_receive_runtime_score_boost() {
        let files: Vec<_> = ["type.py", "runtime.py", "plain.py"].into_iter().map(|path| SourceFile { path: path.into(), lang: crate::lang::Language::Python, kind: FileKind::Source, lines: 1, bytes: 0, content: String::new() }).collect();
        let fm: Vec<_> = files.iter().map(|f| fmetrics(&f.path, 0, 0, 0)).collect();
        let mut deps = DepGraph::default();
        deps.files.insert("type.py".into(), crate::deps::FileDeps { in_cycle: true, ..Default::default() });
        deps.files.insert("runtime.py".into(), crate::deps::FileDeps { in_cycle: true, in_runtime_cycle: true, ..Default::default() });
        let (clones, tests, hcfg, mentions, pcfg) = (CloneReport::default(), TestsCfg::default(), hcfg(), MentionIndex::default(), pcfg());
        let cfg = Cfg { without_history: Weights { hotspot: 0.0, fixes: 0.0, complexity: 0.0, coupling: 1.0, clones: 0.0, size: 0.0, dead: 0.0, strings: 0.0 }, no_tests_multiplier: 1.0, ..Cfg::default() };
        let r = build(Inputs { root: String::new(), files: &files, history: None, file_metrics: &fm, functions: &[], deps: &deps, clones: &clones, cognitive_hard: 15, mentions: &mentions, tests: &tests, list_tables_separately: true, history_cfg: &hcfg, dedupe_cycle_reason: true, plan: &pcfg, dead: nodead(), helpers: nohelpers(), helpers_weight: 0.0, strings: nostrings(), clumps: noclumps(), clumps_weight: 0.0, clumps_prefix: "_", declared: nodeclared(), declared_weight: 0.0, comments: nocomments(), naming: nonaming(), fallback: nofallback() }, 10, &cfg, &td());
        assert_eq!(r.hotspots[0].path, "runtime.py");
        assert_eq!(r.hotspots[0].score, 100.0 * cfg.cycle_coupling);
        assert!(r.hotspots.iter().skip(1).all(|h| h.score == 0.0));
    }
...
pub fn pair_line(c: &ClonePair) -> String {
    ...
    format!("{:>4} tok  {}  <->  {}{}{}", c.tokens, plan::side_text(&c.a, "", table), plan::side_text(&c.b, &c.a.file, table), table_note(c), match c.kind { CloneKind::Signature => " [signature]", CloneKind::Configuration => " [configuration]", _ => "" })
}
...
pub fn render_with(r: &Report, top: usize, plans: bool) -> String {
    ...
    for (i, h) in r.hotspots.iter().take(top).enumerate() {
        ...
        let parts: Vec<_> = h.score_breakdown.contributions.iter().filter(|c| c.points > 0.0)
            .map(|c| format!("{} {:.1}", c.signal, c.points)).collect();
        let _ = writeln!(o, "        score: ({}) x {:.2} test multiplier = {:.1}", parts.join(" + "), h.score_breakdown.test_multiplier, h.score);
        if h.signals.in_cycle {
            let _ = writeln!(o, "        cycle membership: {}", if h.signals.in_runtime_cycle { "runtime" } else { "structural only; no runtime cycle boost" });
        }
        for reason in &h.reasons {
            // A reason's continuation lines (the phase sub-lines) indent under it.
            let mut lines = reason.split('\n');
            if let Some(first) = lines.next() {
                let _ = writeln!(o, "        - {first}");
            }
            for l in lines {
                let _ = writeln!(o, "            {l}");
            }
    ...
    for c in r.file_cycles.iter().take(top) {
        ...
        let runtime: Vec<_> = r.runtime_file_cycles.iter().filter(|members| members.iter().any(|m| c.members.contains(m))).collect();
        if runtime.is_empty() {
            let _ = writeln!(o, "    structural only: no runtime cycle after removing type-only imports");
        } else {
            for members in runtime { let _ = writeln!(o, "    runtime: {}", members.join(" <-> ")); }
        }
    ...
    for family in r.clone_families.iter().take(top) {
        o.push_str(&crate::clones::render_family(family));
    }
    for c in r.clones.iter().filter(|c| !r.clone_families.iter().any(|family| family.matches.iter().any(|q| q.a == c.a && q.b == c.b))).take(top) {
        let _ = writeln!(o, "  {}", pair_line(c));
    }
    if !r.structural_clones.is_empty() {
        let _ = writeln!(o, "  SHARED STRUCTURE  (signatures/configuration; excluded from clone score and extraction plans)");
        for c in r.structural_clones.iter().take(top) { let _ = writeln!(o, "  {}", pair_line(c)); }
    }
